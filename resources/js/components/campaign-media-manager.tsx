...
import { type Campaign, type CampaignMedia } from '@/types';
...
export default function CampaignMediaManager({ campaign, media }: { campaign: Campaign; media: CampaignMedia[] }) {
    ...
    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        post(route('campaigns.media.store', campaign.id), {
            forceFormData: true,
            preserveScroll: true,
            onSuccess: () => {
                reset();
    ...
    const remove = (id: number) => {
        router.delete(route('campaigns.media.destroy', id), { preserveScroll: true });
    };
    ...
    return (
        <div className="rounded-xl border border-border bg-accent/40 p-4">
            <div className="mb-1 flex items-center gap-2">
                ...
                <Label className="text-base">Fotos y videos del anuncio</Label>
            </div>
            <p className="text-muted-foreground mb-4 text-sm">
                El asistente los enviará a los pacientes que lleguen por esta campaña, para generar confianza o cuando pidan ver resultados.
            </p>
            ...
            {media.length > 0 ? (
                ...
                <p className="text-muted-foreground mb-5 rounded-lg border border-dashed border-border/60 px-3 py-6 text-center text-sm">
                    Aún no hay material visual para esta campaña.
                </p>
            )}
            ...
            <form onSubmit={submit} className="flex flex-col gap-3 border-t border-border/60 pt-4">
                <div className="grid gap-3 sm:grid-cols-2">
                    <div className="grid gap-1.5">
                        <Label htmlFor="cmedia_type">Tipo</Label>
                        <select
                            id="cmedia_type"
                            value={data.type}
                ...
                {source === 'file' ? (
                    <div className="grid gap-1.5">
                        <Label htmlFor="cmedia_file">Archivo</Label>
                        <Input
                            id="cmedia_file"
                            type="file"
                    ...
                    <div className="grid gap-1.5">
                        <Label htmlFor="cmedia_url">URL pública del archivo</Label>
                        <Input
                            id="cmedia_url"
                            type="url"
                            value={data.url}
                ...
                <div className="grid gap-1.5">
                    <Label htmlFor="cmedia_caption">Descripción (opcional)</Label>
                    <Input
                        id="cmedia_caption"
                        value={data.caption}
                        onChange={(e) => setData('caption', e.target.value)}
                        placeholder="Ej. Antes y después de la promoción"
                    />
